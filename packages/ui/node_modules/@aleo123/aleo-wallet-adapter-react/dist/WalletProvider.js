import { WalletNotSelectedError, WalletNotConnectedError, WalletNotReadyError, WalletReadyState, DecryptPermission, WalletAdapterNetwork, } from '@aleo123/aleo-wallet-adapter-base';
...
export const WalletProvider = ({ children, wallets: adapters, autoConnect = false, decryptPermission = DecryptPermission.NoDecrypt, network = WalletAdapterNetwork.Testnet, onError, localStorageKey = 'walletName', }) => {
    ...
    useEffect(() => {
        // TODO :以下walletName 暂时使用 soterWallet 后续UI完成之后需要修改
        const wallet = name && wallets.find(({ adapter }) => adapter.name === "Soter Wallet");
        if (wallet) {
            setState({
                wallet,
    ...
    const requestViewKey = useMemo(() => adapter && 'requestViewKey' in adapter
        ? async () => {
            if (!connected)
                throw handleError(new WalletNotConnectedError());
