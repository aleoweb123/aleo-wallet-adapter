import { WalletNotSelectedError, WalletNotConnectedError, WalletNotReadyError, WalletReadyState, DecryptPermission, WalletAdapterNetwork, } from '@demox-labs/aleo-wallet-adapter-base';
...
export const WalletProvider = ({ children, wallets: adapters, autoConnect = false, decryptPermission = DecryptPermission.NoDecrypt, network = WalletAdapterNetwork.Testnet, onError, localStorageKey = 'walletName', programs = [] }) => {
    ...
    const decrypt = useMemo(() => adapter && 'decrypt' in adapter
    ...
    const requestRecords = useMemo(() => adapter && 'requestRecords' in adapter
        ? async (program) => {
            if (!connected)
                throw handleError(new WalletNotConnectedError());
    ...
    const requestTransaction = useMemo(() => adapter && 'requestTransaction' in adapter
        ...
        : undefined, [adapter, handleError, connected]);
    // Request execution
    const requestExecution = useMemo(() => adapter && 'requestExecution' in adapter
        ? async (transaction) => {
            if (!connected)
                throw handleError(new WalletNotConnectedError());
    ...
    const requestBulkTransactions = useMemo(() => adapter && 'requestBulkTransactions' in adapter
        ? async (transactions) => {
            if (!connected)
                throw handleError(new WalletNotConnectedError());
        ...
        : undefined, [adapter, handleError, connected]);
    // Request deploy
    const requestDeploy = useMemo(() => adapter && 'requestDeploy' in adapter
        ? async (deployment) => {
            if (!connected)
                throw handleError(new WalletNotConnectedError());
    ...
    const transactionStatus = useMemo(() => adapter && 'transactionStatus' in adapter
        ? async (transactionId) => {
            if (!connected)
                throw handleError(new WalletNotConnectedError());
        ...
        : undefined, [adapter, handleError, connected]);
    // Request Execution
    const getExecution = useMemo(() => adapter && 'getExecution' in adapter
        ? async (transactionId) => {
            if (!connected)
                throw handleError(new WalletNotConnectedError());
    ...
    const requestRecordPlaintexts = useMemo(() => adapter && 'requestRecordPlaintexts' in adapter
        ? async (program) => {
            if (!connected)
                throw handleError(new WalletNotConnectedError());
    ...
    const requestTransactionHistory = useMemo(() => adapter && 'requestTransactionHistory' in adapter
        ? async (program) => {
            if (!connected)
                throw handleError(new WalletNotConnectedError());
